...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [subscribedFeeds, setSubscribedFeeds] = useLocalStorage<string[]>('rss_subscribed_feeds', []);
  const [bookmarkedPosts, setBookmarkedPosts] = useLocalStorage<string[]>('rss_bookmarked_posts', []);
  ...
  const bookmarkPost = (postId: string) => {
    setBookmarkedPosts(prev => [...prev.filter(id => id !== postId), postId]);
  };
