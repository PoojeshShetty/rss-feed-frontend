...
export function Dashboard({ onNavigate }: DashboardProps) {
  const { user } = useAuth();
  const { blogPosts, feeds, subscribedFeeds, bookmarkedPosts, refreshFeeds, isLoading, bookmarkPost, unbookmarkPost } = useApp();
  ...
  const filteredPosts = useMemo(() => {
    return blogPosts.filter(post => {
      ...
      const isFromSubscribedFeed = subscribedFeeds.includes(post.sourceFeedId);
      if (!user) return true; // Show all posts if not logged in
      if (!isFromSubscribedFeed) return false;

      // Filter by selected feed
      if (selectedFeed !== 'all' && post.sourceFeedId !== selectedFeed) return false;
      ...
      if (searchQuery) {
        ...
        return (
          ...
          post.sourceFeedName.toLowerCase().includes(query)
        );
      }
  ...
  const handleRefresh = async () => {
    try {
      ...
      setMessage({ type: 'error', text: 'Failed to refresh feeds. Please try again.' });
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            {user ? `Welcome back, ${user.displayName}!` : 'Welcome to RSS Reader'}
          </h1>
            ...
            {user && subscribedFeeds.length === 0 ? (
              <div className="max-w-md mx-auto">
                ...
                <p className="text-gray-600 mb-6">
                  You haven't subscribed to any feeds yet. Explore new feeds or add your first one!
                </p>
